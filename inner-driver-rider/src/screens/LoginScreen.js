...
import { login, getMe } from "../api/rider";
  ...
  async function onLogin() {
    ...
    try {
      ...
      if (me.user_type !== "rider") {
        await clearTokens();
        Alert.alert("Wrong app", "This account is not a rider. Use the driver app.");
        return;
      }
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity onPress={() => navigation.navigate("SignUp")} style={styles.linkWrap}>
        <Text style={styles.link}>No account? <Text style={styles.linkStrong}>Sign up as rider</Text></Text>
      </TouchableOpacity>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  button: { backgroundColor: "#2563eb", borderRadius: 12, paddingVertical: 16, alignItems: "center", marginTop: 6 },
